...
> 更新时间：2026-08-16（v9：音频设备切换稳定窗口重建/任务派发可见收口/任务失联看门狗；v10：三分控制中断/幽灵任务槽修复；v11：双 Agent 异步细节加固——详见 §12 双 Agent 异步模型与 §8 批次记录；v12：三分控制中断/幽灵任务槽修复见 §8；v13：专属 SOUL install-dedicated-soul；v14：补齐 0.6 数据归属、隔离边界与升级理念——见 §1.1–§1.3；v15：播报策略「最新优先」——打断清队列/任务过程只出气泡——见 §4）
...
- **打断语义（2026-08-16 修订「最新优先」）**：新 high → stop（停当前播放 + 清 pendingHigh + 作废在途合成 generation+1 + 清低优队列）→ 播新内容——**最终结果优先，中间与排队播报全部丢弃**；唯一例外是轻确认「收到」（clearsQueue=false，不吞队列）
- **说话即打断**：用户开口 → stop **清空低优队列**（原 P3-1「保留队列 2s 后续播」语义废弃——旧任务播报回头续播实测造成话题穿插、听感混乱；细节看气泡/历史）；`recordingGuard` 防录音中插播
...
- 线程收口（2026-08-16）：provider 完成回调与降级 `fallbackSpeak` 统一派发主线程后再动队列/共享状态（NSSound delegate 线程无保证，防并发改队列）
