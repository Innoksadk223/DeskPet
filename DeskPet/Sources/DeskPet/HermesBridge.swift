...
final class HermesBridge {
    ...
    nonisolated static let mainSeedVersion = 5
    ...
    nonisolated static func mainSessionSeed() -> String {
        ...
        4. 回复用双轨格式：先 <spoken>口语精简版——formal 全量信息的浓缩，覆盖每条核心信息，宁短勿空、不丢点</spoken>，再 <formal>书面完整版（可含 Markdown/代码，供气泡展示）</formal>。普通闲聊 <formal> 可省略。spoken 是 formal 的精简浓缩，不是开头/预告/引子：每条核心信息都要浓缩提及（例：formal 写「他十分爽快地笑了一声，哈哈哈」，spoken 应说「他爽朗地笑了」）；禁止「我把…讲给你听」「下面细说」这类过渡句。
           spoken 是「说给耳朵听的话」，必须纯净可朗读：禁止括号动作/神态/舞台指示（如（歪着头想了想）（笑着挥手）——那是演给眼睛看的，只放 formal）；禁止 emoji、Markdown 符号（**、#、列表符号）、表格、代码、链接；动作与情绪直接融进台词本身。违反任何一条，用户听到的就是混乱噪音。
    ...
    nonisolated static func taskSessionSeed() -> String {
        ...
        回复协议（必须遵守）：
        1. 完成后用双轨格式输出：<spoken>精简播报版——formal 全量信息的浓缩，覆盖每条核心信息（例：formal 写「他十分爽快地笑了一声，哈哈哈」，spoken 应说「他爽朗地笑了」）；禁止「我把…讲给你听」这类过渡开头，禁止只念开头。spoken 是「说给耳朵听的话」，必须纯净可朗读：禁止括号动作/神态描写、emoji、Markdown 符号、代码与表格标记</spoken>，再 <formal>完整正式结果（可含代码/表格/Markdown，供气泡展示）</formal>。
